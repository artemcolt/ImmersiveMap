...
final class ImmersiveMapRenderCamera {
    ...
    private let cameraSynchronizer: RenderCameraSynchronizer
    ...
    init(settings: ImmersiveMapSettings) {
        ...
        self.cameraSynchronizer = RenderCameraSynchronizer()
    ...
    func makeFrameState(drawSize: CGSize, diagnostics: FrameDiagnostics) -> CameraFrameState? {
        ...
        cameraSynchronizer.updateIfNeeded(camera: camera, cameraControl: cameraControl)
    ...
    func setCenterWorldMercator(_ centerWorldMercator: SIMD2<Double>) {
        cameraControl.setCenterWorldMercator(centerWorldMercator)
        requestRenderCameraUpdate()
    }
    ...
    func applyCameraSettings(_ settings: ImmersiveMapSettings.CameraSettings) {
        cameraControl.apply(settings: settings)
        requestRenderCameraUpdate()
    }

    func applyConstraints(_ constraints: RenderCameraConstraints) {
        cameraControl.clampBearing(to: constraints.bearing)
        cameraControl.clampPitch(to: constraints.pitch)
        requestRenderCameraUpdate()
    }
    ...
    private func requestRenderCameraUpdate() {
        cameraSynchronizer.requestUpdate()
    }
}

...
class CameraControl {
    ...
    private(set) var cameraState: MapCameraState = .default
    private var settings: MapSettings.CameraSettings

    var globePan: SIMD2<Double> {
        MapProjection.globePan(fromCenterWorldMercator: cameraState.centerWorldMercator)
    }

    var flatPan: SIMD2<Double> {
        MapProjection.flatPan(fromCenterWorldMercator: cameraState.centerWorldMercator)
    }
    ...
    init(settings: MapSettings.CameraSettings) {
        self.settings = settings
    }

    convenience init(config: MapSettings) {
        self.init(settings: config.camera)
    }

    func apply(settings: MapSettings.CameraSettings) {
        self.settings = settings
    ...
    func setCenterWorldMercator(_ centerWorldMercator: SIMD2<Double>) {
        cameraState.centerWorldMercator = SIMD2<Double>(MapProjection.wrapNormalizedWorldX(centerWorldMercator.x),
    ...
    func setLatLonDeg(latDeg: Double, lonDeg: Double) {
        ...
        setCenterWorldMercator(MapProjection.worldMercator(latitude: globeLat, longitude: longitude))
        update = true
    }
    ...
    func getLatLonRad() -> (latRad: Double, lonRad: Double) {
        let latRad = MapProjection.latitude(fromNormalizedWorldY: cameraState.centerWorldMercator.y)
        let lonRad = MapProjection.longitude(fromNormalizedWorldX: cameraState.centerWorldMercator.x)
    ...
    func setCameraPosition(_ cameraPosition: ImmersiveMapCameraPosition) {
        ...
        cameraState.centerWorldMercator = MapProjection.worldMercator(latitude: latitudeRadians,
    ...
    func setCameraState(_ cameraState: MapCameraState) {
        ...
        self.cameraState = MapCameraState(centerWorldMercator: cameraState.centerWorldMercator,
                                          zoom: clampedZoom,
